...
export default function Testimonials() {
  ...
  // Autoplay sobrio (se pausa al interactuar implícitamente al re-render)
  useEffect(() => {
        ...
        <div className="mx-auto max-w-2xl">
          ...
          <div className="mt-6 flex items-center justify-center gap-4">
            ...
            <div className="flex gap-2">
              {ITEMS.map((_, idx) => (
                <button key={idx} aria-label={`Ir al testimonio ${idx + 1}`} onClick={() => setI(idx)}
                  className={`h-2.5 rounded-full transition-all ${idx === i ? "w-6 bg-blue" : "w-2.5 bg-line"}`} />
              ))}
            </div>
