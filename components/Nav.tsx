    ...
    <header className="sticky top-0 z-50 border-b border-line bg-white/85 backdrop-blur-md">
      <div className="wrap flex h-[74px] items-center justify-between">
        ...
        <nav className="hidden items-center gap-7 lg:flex">
          ...
          <Link href="/contacto" className="btn btn-primary !px-5 !py-2.5">Solicitar informes</Link>
        </nav>
        <button aria-label="Menú" onClick={() => setOpen((v) => !v)} className="flex flex-col gap-[5px] lg:hidden">
      ...
      {open && (
        <div className="border-t border-line bg-white px-6 py-4 lg:hidden">
          <div className="flex flex-col gap-4">
            ...
            <Link href="/contacto" onClick={() => setOpen(false)} className="btn btn-primary">Solicitar informes</Link>
          </div>
        </div>
      )}
    </header>
